...
public class ExcelUtil {
    ...
    * @throws 
    */
    public static <T> List<T> importDataFromExcel(T vo,InputStream is,String excelFileName){
        ...
        try {
            ...
            for (int i = 1; i < rows; i++) {
                T newVO = (T) vo.getClass().getConstructor(new Class[] {}).newInstance(new Object[] {});
                //第一行为标题栏，从第二行开始取数据
                ...
                while (index < cells) {
                    ...
                    Method setMethod = newVO.getClass().getMethod(methodName, new Class[]{String.class});
                    setMethod.invoke(newVO, new Object[]{value});
                    index++;
                }
                //判断对象属性是否有值
                if (isHasValues(newVO)) {
                    list.add(newVO);
                    //重新创建一个vo对象
                    //Object o = vo.getClass().getConstructor(new Class[] {}).newInstance(new Object[] {});
                }
